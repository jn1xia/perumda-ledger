...
const APR_FILE = 'LAMPIRAN LAPORAN KEUANGAN APRIL 2026.xlsx';
const APR_SHEET = 'JURNAL APRIL 2026';
...
async function getDbBalances(db) {
  return new Promise((resolve, reject) => {
    db.all(`SELECT akun_debit, akun_kredit, debit, kredit FROM journals WHERE tanggal LIKE '2026-04%' AND status='posted'`, (err, rows) => {
...
async function main() {
  ...
  const xlBalances = getExcelBalances(path.join(DIR, APR_FILE), APR_SHEET, nameLookup);
  
  const allKeys = new Set([...Object.keys(dbBalances), ...Object.keys(xlBalances || {})]);
  
  let diffCount = 0;
  console.log("=== DIFFERENCES (DB vs EXCEL APR) ===");
