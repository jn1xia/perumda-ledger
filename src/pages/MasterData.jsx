...
import { Plus, Search, Pencil, Trash2, X, Save, Users, Truck, Download, Printer, Building2 } from 'lucide-react'
...
const emptyDepartemen = { kode: '', nama: '', keterangan: '' }

export default function MasterData() {
  const { state, dispatch } = useApp()
  const pelangganList  = state.pelangganMaster || []
  const supplierList   = state.supplierMaster  || []
  const departemenList = state.departemen       || []

  const [tab, setTab]           = useState('pelanggan')
  const [search, setSearch]     = useState('')
  const [showModal, setShowModal] = useState(false)
  const [editId, setEditId]     = useState(null)   // kode for departemen, id for pelanggan/supplier
  const [form, setForm]         = useState({ ...emptyPelanggan })

  const isDepartemen = tab === 'departemen'
  const isPelanggan  = tab === 'pelanggan'

  const list = isDepartemen ? departemenList : isPelanggan ? pelangganList : supplierList
  ...
  function openAdd() {
    setForm(isDepartemen ? { ...emptyDepartemen } : isPelanggan ? { ...emptyPelanggan } : { ...emptySupplier })
    setEditId(null)
    setShowModal(true)
  }
  ...
  function openEdit(item) {
    setForm({ ...item })
    setEditId(isDepartemen ? item.kode : item.id)
    setShowModal(true)
  }
  ...
  function handleSave() {
    ...
    if (isDepartemen) {
      if (!form.kode) return alert('Kode departemen wajib diisi')
      if (editId) {
        dispatch({ type: 'UPDATE_DEPARTEMEN', payload: { ...form, kode: editId } })
      } else {
        dispatch({ type: 'ADD_DEPARTEMEN', payload: form })
      }
    } else if (editId) {
  ...
  function handleDelete(item) {
    const label = isDepartemen ? 'departemen' : isPelanggan ? 'pelanggan' : 'supplier'
    if (!confirm(`Hapus ${label} ini?`)) return
    if (isDepartemen) {
      dispatch({ type: 'DELETE_DEPARTEMEN', payload: item.kode })
    } else {
      dispatch({ type: isPelanggan ? 'DELETE_PELANGGAN' : 'DELETE_SUPPLIER', payload: item.id })
    }
  }

  function handleExport() {
    if (isDepartemen) {
      exportCSV('Master_Departemen', ['Kode', 'Nama', 'Keterangan'],
        departemenList.map(d => [d.kode, d.nama, d.keterangan]))
    } else {
  ...
  const tabCounts = {
    ...
    supplier:   supplierList.length,
    departemen: departemenList.length,
  }
  ...
  return (
    <div className="animate-in">
      <div className="page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1>Master Data</h1>
          <p>{pelangganList.length} pelanggan · {supplierList.length} supplier · {departemenList.length} departemen</p>
        </div>
      ...
      <div className="tabs" style={{ marginBottom: 16 }}>
        {TABS.map(t => (
          <button key={t.id} className={`tab ${tab === t.id ? 'active' : ''}`} onClick={() => { setTab(t.id); setSearch('') }}>
            ...
            <span className="badge blue" style={{ marginLeft: 6, fontSize: 11 }}>{tabCounts[t.id]}</span>
          </button>
        ))}
      </div>
      ...
      <div className="toolbar">
        <div className="topbar-search" style={{ maxWidth: 300 }}>
          ...
          <input
            type="text"
            placeholder={`Cari ${isDepartemen ? 'departemen' : isPelanggan ? 'pelanggan' : 'supplier'}...`}
            value={search}
        ...
        <div className="toolbar-right">
          <button className="btn btn-primary" onClick={openAdd}>
            <Plus size={16} /> Tambah {isDepartemen ? 'Departemen' : isPelanggan ? 'Pelanggan' : 'Supplier'}
          </button>
        </div>
      </div>
        ...
        <div className="card">
          <div className="table-container">
            <table>
              <thead>
                <tr>
                  <th>Kode</th><th>Nama Departemen</th><th>Keterangan</th>
              ...
              <tbody>
                {filtered.map(d => (
                  <tr key={d.kode}>
                    <td className="mono" style={{ fontWeight: 600 }}>{d.kode}</td>
                    <td style={{ fontWeight: 500 }}>{d.nama}</td>
                    <td style={{ fontSize: 12, color: 'var(--text-muted)' }}>{d.keterangan || '-'}</td>
                    <td className="text-center">
                      <div style={{ display: 'flex', gap: 4, justifyContent: 'center' }}>
                        <button className="btn btn-sm btn-outline" onClick={() => openEdit(d)}><Pencil size={12} /></button>
                        <button className="btn btn-sm btn-outline" style={{ color: 'var(--danger)' }} onClick={() => handleDelete(d)}><Trash2 size={12} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
                {filtered.length === 0 && (
                  <tr><td colSpan={4} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>
                    Belum ada data departemen. Klik "+ Tambah Departemen" untuk memulai.
                  </td></tr>
                )}
              </tbody>
            </table>
              ...
              <tbody>
                  ...
                  <tr key={item.id}>
                    ...
                    <td className="text-center">
                      <div style={{ display: 'flex', gap: 4, justifyContent: 'center' }}>
                        ...
                        <button className="btn btn-sm btn-outline" style={{ color: 'var(--danger)' }} onClick={() => handleDelete(item)}><Trash2 size={12} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
                {filtered.length === 0 && (
                  <tr><td colSpan={9} style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>
                    Belum ada data {isPelanggan ? 'pelanggan' : 'supplier'}
                  </td></tr>
                )}
              </tbody>
            </table>
      ...
      {showModal && (
        <Modal
          title={`${editId ? 'Edit' : 'Tambah'} ${isDepartemen ? 'Departemen' : isPelanggan ? 'Pelanggan' : 'Supplier'}`}
          onClose={() => setShowModal(false)}
          width={isDepartemen ? 480 : 640}
          footer={
            <>
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  ...
                  <input
                    ...
                    placeholder="Pasar Sentra Antasari"
                  />
                </div>
              </div>
              <div className="form-group">
                ...
                <input
                  ...
                  placeholder="Cost center / deskripsi departemen"
                />
              </div>
            </>
          ) : (
            /* Pelanggan / Supplier form */
            <>
              <div className="form-row">
                <div className="form-group">
                  <label className="form-label">Kode</label>
