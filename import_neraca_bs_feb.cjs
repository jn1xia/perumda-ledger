...
async function main() {
  console.log('=== Importing Missing Balance Sheet Accounts for Feb 2026 ===\n');
  const db = new sqlite3.Database(DB_PATH);
  
  // Clear any previously imported Neraca-only SUM entries if we ran this before
  const del = await dbRun(db, "DELETE FROM journals WHERE id LIKE 'SUM-2026-02-BS-%'");
